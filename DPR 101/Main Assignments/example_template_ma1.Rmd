...
Data |>
  ...
  labs(
    ...
    y = '"Yes" on Issue 1 vs. Trump',
    subtitle = "Figure 4: Median Income and Issue 1 Underperformance",
    size = "County\nPopulation"
  )
```
...
library(estimatr)
lm_robust(
  yes_v_trump ~ turnoutpct + collegepct + log(median_income) + 
    log(population) +
    malepct + whitepct,
  data = Data |> 
    mutate(
  ...
  geom_pointrange(
    aes(alpha = ifelse(p.value < 0.05, "yes", "no")),
    size = .2
  ) +
  geom_text(
