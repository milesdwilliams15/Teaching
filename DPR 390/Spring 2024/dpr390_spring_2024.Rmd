...
\item Day 2: `{peacesciencer}` workshop with group activity.

\item Day 3: `{peacesceincer}` workshop with group activity.
...
## `r advdate(mon, 4)`: Unchecked Interests
...
\end{multicols}
```
## `r advdate(mon, 5)`: Intangible Incentives
...
\end{multicols}
```
## `r advdate(mon, 7)`: Commitment Problems
...
\end{multicols}
```
## `r advdate(mon, 8)`: Misperceptions
...
## `r advdate(mon, 10)`: Interdependence
...
\end{multicols}
```
## `r advdate(mon, 11)`: Checks and Balances
...
\end{multicols}
```
## `r advdate(mon, 12)`: Rules and Enforcement
...
\end{multicols}
```
## `r advdate(mon, 13)`: Interventions
...
\end{multicols}
```
## `r advdate(mon, 14)`: What Can We Do?
...
\item Day 1: Read WWW "Chapter 11: Wayward Paths to Peace."

\item Day 2: Using data to talk about gender, poverty, and other factors.

\item Day 3: Read WWW "Chapter 12: The Peacemeal Engineer."
...
## `r advdate(mon, 15)`: Review and Final Projects
...
\item Day 1: Final project workshop.

\item Day 2: Final project workshop.

\item Day 3: Presentations.
...
We'll also work with R and RStudio in this course. There are many free and helpful resources online, but if you're looking for some authoritative references, I highly recommend:
...
During my office hours, my door is always open! If you have any questions or concerns about the course, just drop by my office Monday or Wednesday between 2:30-4:00 pm. No appointment necessary. If those hours don't work for you, we can work out a different time to meet.
...
DPR hires teaching assistants each semester to provide you with extra help and tutoring. Since the TA hours will change from semester to semester, details like how to contact the TAs, their location, and their office hours are TBD until the semester starts.
...
15\%  & \text{ (Weekly Reflection)} \\
45\%  & \text{ (Data Challenges)} \\
\underline{+ \space 30\%} &  \text{ (Final Project)} \\
